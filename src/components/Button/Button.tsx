...
`
    ...
    return (
      <Comp
        ref={ref}
        target={target}
        className={`${_outline} ${_color} ${text ? 'hover:underline' : ''} ${_textColor} ${font} ${spacing} ${className} ${_rounded}  ${large ? 'w-full' : ''}  flex align-middle  ${left ? 'items-start' : 'items-center'} ${right ? 'items-end' : 'items-center'} select-none hover:bg-opacity-80 ${disable ? 'pointer-events-none cursor-not-allowed bg-opacity-50' : ''} ${children ? `` : 'w-fit p-2 aspect-square flex justify-center'}`}
        {...props}
      >
